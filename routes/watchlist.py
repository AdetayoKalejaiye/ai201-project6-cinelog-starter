...
def add_film(user_id):
    ...
    except AlreadyInWatchlistError:
        return jsonify({"error": "Film is already on this user's watchlist"}), 409
...
def remove_film(user_id):
    ...
    removed = remove_from_watchlist(user_id=user_id, film_id=data["film_id"])
    if not removed:
        return jsonify({"error": "Film is not on this user's watchlist"}), 404
